namespace RaceboxIntegration.Other
{
    ...
    public class ControllerFactory
    {
        ...
        /// <returns>An IDeviceController implementation specific to the device type, or a GenericDeviceController if the device type is unknown.</returns>
        public static IDeviceController CreateController(string deviceName)
        {
            if (deviceName.ToLowerInvariant().Contains("racebox"))
            {
                return new RaceboxDeviceController();
            }
