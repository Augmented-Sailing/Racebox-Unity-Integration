...
namespace RaceboxIntegration.UI
{
    ...
    public class ConnectionComponent : MonoBehaviour
    {
        ...
        private readonly List<DeviceEntry> _bluetoothEntries = new();
        ...
        private void ClearEntries()
        {
            ...
            foreach (Transform child in _deviceList) Destroy(child.gameObject);
        }
        ...
        {
            var devices = BluetoothManager.Instance.GetDeviceList();
            ...
            {
                if (RefreshDevice(device.DeviceUID)) continue;
        ...
        {
            var entryFound = _bluetoothEntries.FirstOrDefault(x => x.BluetoothDevice.DeviceUID == device.DeviceUID);
            if (entryFound != null)
            {
                entryFound.Refresh();
                return;
            ...
            var entryObject = Instantiate(_deviceEntryPrefab, _deviceList);
            var entry = entryObject.GetComponent<DeviceEntry>();
